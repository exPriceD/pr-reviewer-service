...
Структура БД описана в `migrations/000001_init_schema.up.sql`. Ключевые ограничения:

- `pull_requests.author_id` и `pr_reviewers.user_id` ссылаются на `users.user_id` с `ON DELETE RESTRICT`, поэтому удалить автора активного PR нельзя.
- `pr_reviewers` содержит композитный ключ `(pull_request_id, user_id)`, который не позволяет назначить ревьювера повторно.
- Индексы:
  - `idx_users_team_name` и частичный `idx_users_team_active (team_name) WHERE is_active = TRUE` - быстрые выборки участников и активных кандидатов для ревью;
  - `idx_pr_reviewers_user` и `idx_pr_reviewers_pr` - мгновенный доступ к PR по ревьюверу и наоборот;
  - `idx_pr_author`, `idx_pr_status`, `idx_pr_created_at` - статистика, фильтрация по статусу и сортировка PR.
  - Эти индексы покрывают основные пути чтения (`/team/get`, `/users/getReview`, `/statistics`)

### Менеджер транзакций

Все use case оборачиваются в `txManager.Do` из пакета `github.com/avito-tech/go-transaction-manager`. Менеджер хранит `*sql.Tx` в `context.Context`, позволяет переиспользовать соединение между репозиториями и гарантирует атомарность операций (например, при создании PR сразу записываются сам PR и его ревьюверы). Благодаря этому бизнес-логика не зависит от конкретной реализации транзакций и остаётся чистой.

### Массовая деактивация и batch-операции

Чтобы избежать N+1 обновлений при деактивации команды, реализован метод `BatchDeactivateByTeamName`, который выполняет один запрос:
...
Это позволяет укладываться в SLA даже при 200 пользователях.

### Переназначение ревьювера

Вместо удаления всех ревьюверов и повторной вставки используется точечный `UPDATE`:
...
Если affected rows = 0, возвращается доменная ошибка `ErrReviewerNotFound`.
...
YAML-конфиги дают базовые значения, а окружение (`DB_HOST`, `SERVER_HOST`, `CONFIG_FILE`) переопределяет их. Такой порядок позволяет запускать сервис локально, в Docker и на CI без правки файлов.
...
#### 4. Когда операция `/pullRequest/reassign` возвращает `409 NO_CANDIDATE`?

Ответ: Когда после фильтрации деактивированных и уже назначенных пользователей в команде не остаётся кандидатов. Это не ошибка сервера - бизнес-ограничение. Такое поведение пришлось зафиксировать отдельно, чтобы нагрузочные тесты и клиенты не воспринимали `409` как сбой.

#### 5. Что происходит, если указать `old_reviewer_id`, который не привязан к PR?

Ответ: Возвращается `409 NOT_ASSIGNED`. Репозиторий `ReplaceReviewer` проверяет количество обновлённых строк и, если ревьювер не найден, возвращает `ErrReviewerNotAssigned`, который мапится на соответствующий код. Этот сценарий не был описан в спецификации, но встречается в реальной эксплуатации (например, если фронт успел показать устаревший список ревьюверов).

#### 6. Что будет, если вызвать `/team/deactivateMembers` для несуществующей команды?

Ответ: Возвращается `404 NOT_FOUND`. В исходном описании этот случай не упоминался, поэтому мы уточнили: перед массовой деактивацией команда ищется в репозитории, и если её нет, возвращается `ErrTeamNotFound`.
